...
ALL_TOOLS: Dict[str, Dict[str, Any]] = {
    ...
    "edit_html": {
        ...
        "function": {
            "name": "edit_html",
            "description": "Edit an HTML page in the workspace by replacing one piece of text with another. Use this to update the headline, a button label, a tagline, etc. on a page that was previously generated by html_assistant. The change is applied to the file and the live preview re-renders.",
            "parameters": {
                "type": "object",
                "properties": {
                    "old_text": {"type": "string", "description": "Exact text currently on the page to replace. Must match a unique substring in the HTML."},
                    "new_text": {"type": "string", "description": "Text to replace it with."},
                    "filename": {"type": "string", "description": "Workspace path of the HTML file to edit. Leave empty to edit the default landing page (workspace/landing.html); only set this when editing a different page.", "default": "workspace/landing.html"},
                },
...
async def _tool_edit_html(args: Dict[str, Any]) -> str:
    old_text = args.get("old_text", "")
    new_text = args.get("new_text", "")
    filename = args.get("filename") or "workspace/landing.html"
    try:
        p = _resolve_safe(filename)
        if not p.exists():
            return json.dumps({"error": f"{filename} not found; generate it first via html_assistant"})
        content = p.read_text(encoding="utf-8")
        count = content.count(old_text)
        if count == 0:
            return json.dumps({"error": "old_text not found in file"})
        if count > 1:
            return json.dumps({"error": f"old_text matches {count} locations; please include more surrounding context"})
        new_content = content.replace(old_text, new_text, 1)
        p.write_text(new_content, encoding="utf-8")
        return json.dumps({"status": "ok", "html": new_content})
    except Exception as e:
        return json.dumps({"error": str(e)})
